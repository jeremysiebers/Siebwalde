...
static udpTrans_t udpRecvBox[8];
static udpTrans_t udpTransBox[8];
...
_Bool CheckDataInSendMailBox (void);
# 61 "communication.c"
void COMMxSETxRXxDATAxHANDLER(void (* RxDataHandler)(void)){
    ...
    switch(ethernetState){
        ...
        case ETHERNET_CONNECT:
            ...
            if(CHECKxDATAxINxRECEIVExMAILxBOX()){
                udpPacketHeader.destinationAddress = (ipv4Header.srcIpAddress);
                udpPacketHeader.destinationPortNumber = (destPort);
                udpPacketHeader.sourceAddress = (ipv4Header.dstIpAddress);
                udpPacketHeader.sourcePortNumber = (udpHeader.dstPort);

                ret = UDP_Start(udpPacketHeader.destinationAddress, udpPacketHeader.sourcePortNumber, 60000);
                if(ret == SUCCESS){
                    isUdpConnected = 1;
        ...
        case ETHERNET_DATA_TX:
            ...
            if(ret == SUCCESS){
                if (CheckDataInSendMailBox()){
                    ...
                    ETH_WriteBlock((char *)udpSend,udpTrans_t_length);
                    UDP_Send();
                }
            }
            else{
...
void UDPxDATAxRECV(int16_t length)
{
    if(length == udpTrans_t_length){
        ETH_ReadBlock(&udpReceivedData,udpTrans_t_length);
...
void PutDataInReceiveMailBox (udpTrans_t data){
    ...
    if(data.header == HEADER){
        *M_Box_Eth_Recv_Ptr = data;
        M_Box_Eth_Recv_Ptr++;
        if(M_Box_Eth_Recv_Ptr >= &udpRecvBox[8]){
            M_Box_Eth_Recv_Ptr = &udpRecvBox[0];
        }
    }
}
...
udpTrans_t *GETxDATAxFROMxRECEIVExMAILxBOX(){
    ...
    if (M_Box_Eth_Recv_Ptr != M_Box_Eth_Recv_Ptr_prev){
        ...
        if(M_Box_Eth_Recv_Ptr_prev >= &udpRecvBox[8]){
            M_Box_Eth_Recv_Ptr_prev = &udpRecvBox[0];
        }
    }
    return (data);
...
# 284 "communication.c"
_Bool CHECKxDATAxINxRECEIVExMAILxBOX(){
...
udpTrans_t *GetDataFromSendMailBox (){
    ...
    if (M_Box_Eth_Send_Ptr != M_Box_Eth_Send_Ptr_prev){
        ...
        if(M_Box_Eth_Send_Ptr_prev >= &udpTransBox[8]){
...
void PUTxDATAxINxSENDxMAILxBOX (udpTrans_t *data){
    ...
    *M_Box_Eth_Send_Ptr = *data;
    ...
    if(M_Box_Eth_Send_Ptr >= &udpTransBox[8]){
