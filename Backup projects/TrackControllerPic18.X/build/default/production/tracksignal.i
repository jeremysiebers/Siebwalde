    ...
    switch(signal){
        case SIG_RED:
            if(path == TRACK1|| path == TRACK10){
                *self->setSignal->port1_ptr &= ~pin1;
                *self->setSignal->port2_ptr |= pin2;
            }
            else if(path == TRACK2 || path == TRACK11){
                *self->setSignal->port3_ptr &= ~pin3;
                *self->setSignal->port4_ptr |= pin4;
            }
            else if(path == TRACK3 || path == TRACK12){
                *self->setSignal->port5_ptr &= ~pin5;
                *self->setSignal->port6_ptr |= pin6;
            }
            break;
