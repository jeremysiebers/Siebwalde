...
class netlist():
    ...
    def groupComponentsfarnell(self, components = None):
        """Return a list of component lists. Components are grouped together
        when the value, library and part identifiers match.
        ...
        # Group components based on the value, library and part identifiers
        for c in components:
            if c.grouped == False and c.getField("Farnell", False) != '':
                c.grouped = True
                newgroup = []
                newgroup.append(c)
                ...
                # and add to the group as necessary
                for ci in components:
                    if ci.grouped == False and ci.getField("Farnell") == c.getField("Farnell"):
                        newgroup.append(ci)
                        ci.grouped = True
