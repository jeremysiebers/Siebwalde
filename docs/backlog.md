...
| Add the `CoreSettings` section to the WPF `App.config`. | The WPF `App.config` only defines `SiebwaldeApp.Properties.Settings`; `CoreSettings` values fall back to Designer defaults. | The WPF app.config carries explicit `SiebwaldeApp.Core.Properties.CoreSettings` values. |
| Decide the role of `SiebwaldeApp.Core.Host/Program - Copy.cs`. | Excluded from compilation via `<Compile Remove>` in the host csproj; stale copy of `Program.cs`. | File is removed or documented as intentionally kept. |
