...
| Item | Evidence | Acceptance criteria |
| --- | --- | --- |
| **SAFETY INVESTIGATION (HIGH PRIORITY) - physical stop reachability after locomotive block mapping is lost.** Source-confirmed safety reachability gap; targeted hardware reproduction pending. | Source evidence (software review 2026-09-20): a locomotive may already be physically moving on an amplifier; if its block/amplifier mapping subsequently becomes unavailable, `EcosHardwareStopSink` attempts to stop through the normal loco/backend path, but `TrackAmplifierHardwareBackend.SetLocoSpeed` / target resolution can return `false` because there is no longer a mapped physical target, so the normal safety-stop path may no longer be able to reach the amplifier that was driving the locomotive. | Answer: can the mapping actually disappear while an amplifier is still driving non-neutral PWM in normal runtime? does the amplifier retain its previous setpoint? can `EcosHardwareStopSink` identify the last commanded physical amplifier by another path? if not, what state must be retained so an emergency/safety stop can always reach the last commanded actuator? Potential future architectural direction (NOT yet approved): retain sufficient `last-commanded physical target` state for stop/neutralization independently of the current route/block mapping. Do not implement now. |
