...
| Decide the Page-Removed legacy XAML leftovers. | `TrackAmplifierItemView.xaml`, `TrackAmplifierManualControlView.xaml`, and `TrackControlView.xaml` are excluded from compilation via `<Page Remove>` in `SiebwaldeApp.csproj`; `TrackAmplifierItemView.xaml` now has no code-behind. | Each leftover is removed (with its csproj entries) or documented as intentionally kept. |
