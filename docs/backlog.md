...
| **UNCONFIRMED LIVE OBSERVATION / requires targeted reproduction: a direction command issued while a locomotive has no known block may be lost, so the locomotive could start in the wrong direction.** | Live validation 2026-09-20: loc2's `set(1001,dir[0])` at 12:37:30 was refused with `<END 8 (SAFETY_INTERLOCK)>` because the locomotive had no known block, and `Direction` appeared to stay at its default `1` (reverse) when loc2 was later placed in block 3. The behaviour was **not** reproduced during the later successful live validation, so it is not a confirmed defect. | Targeted reproduction (Integrator) to determine whether the source already retains direction correctly; only a `CONFIRMED PRODUCT DEFECT` verdict creates a Developer task. Do not change production code before that. |
