...
| Real occupancy integration | Completed | `IOccupancyProvider.IsBlockOccupancyKnown`, `TrackAmplifierOccupancyProvider`, `AmplifierOccupancyObservability`, `TrackAmplifierDataFreshness`; verified against `processio.c`/`TrackCommClientAsync`. | Not yet exercised against live amplifiers; the 2 s freshness window may need tuning. |
