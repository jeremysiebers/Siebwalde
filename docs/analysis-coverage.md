...
| Unit tests | Implemented | `SiebwaldeApp.Core.Tests`, 227 tests passing. | UI-model and other areas still uncovered. |
...
| DCC28 speed scaling | **Confirmed defect, not fixed** | Live: protocol `DCC28`, steps `0..28`, step 24 -> ~PWM 475 while the mapper scales by 127. | Normalize at the ECoS/protocol boundary; regression test. |
