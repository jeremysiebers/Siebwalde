...
| Direction retention while unmapped | **Confirmed defect 2026-09-20** | Targeted live reproduction: `set(1001,dir[...])` was refused with `<END 8 (SAFETY_INTERLOCK)>` while loco 2 had no known block and no `dir[...]` event was emitted; after block 1 was assigned, `speedstep[1]` wrote reverse-band PWM 382 (0x017E) while Koploper showed forward. Root cause `SimpleEcosBackend.cs:438-454` (direction stored only on hardware success) + `TrackAmplifierHardwareBackend.cs:89-100`. | Developer fix + focused unit test; not started (awaiting Product Owner approval). |
