...
| Unit tests | Implemented | `SiebwaldeApp.Core.Tests`, 280 tests passing (Debug and Release). | UI-model and other areas still uncovered. |
...
| Stop reachability after block mapping loss | **Physically confirmed defect 2026-09-20** | Live validation (committed harness `03f5221`, branch `feature/safety-stop-reachability`): after a real logical block 1 -> block 3 transition, amp 1 stayed at HR0 416 with no neutral write; the real loco-scoped safety stop neutralized the resolved amp 3 to 399 but left amp 1 at 416, while `EcosHardwareStopSink.StopLoco` returned `True` and no failure diagnostic was emitted (`STOP REACHABILITY DEFECT CONFIRMED`, `STOP FAILURE REPORTING DEFECT CONFIRMED`). The layout stop `SetPower(false)` then neutralized amp 1 to 399 (`LAYOUT STOP FALLBACK PASS`); amp 6 (installed, unmapped) untargeted. | Developer safety-fix task not started: last-commanded physical target retention + honoured stop result + amplifier-centric escalation. |
