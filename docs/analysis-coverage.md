...
| Unit tests | Implemented | `SiebwaldeApp.Core.Tests`, 267 tests passing (Debug and Release). | UI-model and other areas still uncovered. |
| Live Koploper setpoint path | Validated on hardware | 2026-09-19: `set(1000,speedstep[n])` on 15471 reached the amplifier (`[HR0] amp 1 PWM = 469/475`), motor responded to the hand controller, ~120 ms C# -> amplifier. | The standalone harness initially omitted `TrackControlMain.StartRuntime` (harness limitation, not a product defect). |
| DCC28 speed scaling | **Fixed in software 2026-09-20; live verification pending** | `ProtocolSpeedNormalizer` (DCC28 `0..28` -> `0..127`, round-half-up) called from `SimpleEcosBackend`; `ProtocolSpeedNormalizerTests`, `SimpleEcosBackendSpeedNormalizationTests`. Root cause: the `speed` branch also matched `speedstep[...]`. | Live DCC28 full-range verification through the amplifier (Integrator). |
| Live occupancy bridge (`TrackAmplifierOccupancyBridge` -> module 100 -> Koploper bezetmelder) | **Not yet validated** | The ECoS/Koploper bridge was not exercised live in this session. | Full chain validated live with all loco speeds at zero. |
| Real occupancy integration | Completed | `IOccupancyProvider.IsBlockOccupancyKnown`, `TrackAmplifierOccupancyProvider`, `AmplifierOccupancyObservability`, `TrackAmplifierDataFreshness`. **Physically validated 2026-09-19** on amplifiers 1/3/4/6: bit 10 follows occupancy, intervals median 40-42 ms / max 66-70 ms, and a real communication loss made blocks unknown instead of clear. | Occupancy transition latency, WPF indication comparison, the ECoS/Koploper bridge and occupancy-driven safety with a real train are not yet validated. |
