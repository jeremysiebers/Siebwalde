...
| Unit tests | Implemented | `SiebwaldeApp.Core.Tests`, 218 tests passing. | UI-model and other areas still uncovered. |
| Real occupancy integration | Completed | `IOccupancyProvider.IsBlockOccupancyKnown`, `TrackAmplifierOccupancyProvider` reading `TrackAmplifierItem`, `AmplifierOccupancyObservability`; verified against `processio.c`/`TrackCommClientAsync`. | Not yet exercised against live amplifiers. |
